...
PasswordPassage::PasswordPassage(const std::string& name, const std::string& description, Room* from, Room* to, const std::string& password)
    : Passage(name, description, from, to), password(password) {
    locked = true;
}
...
bool PasswordPassage::tryUnlock(const std::string& inputPassword) {
    if (inputPassword == password) {
        ...
        std::string otherName = toRoom->getName() + "_to_" + fromRoom->getName();
